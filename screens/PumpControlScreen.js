...
export default function PumpControlScreen({ navigation }) {
  ...
  // Function to handle turning the pump on/off
  const togglePump = async () => {
    try {
      // Call API to toggle pump status
      const response = await fetch('https://io.adafruit.com/api/v2/longthangtran/feeds/iot-pump/data/last?X-AIO-Key=aio_vfoS43ND1LSwYHLg3zWG1AVjoyPV', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ isOn: !isPumpOn }),  // Send current pump status to server
      });
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity
          style={[styles.button, { backgroundColor: isPumpOn ? "#F44336" : "#4CAF50" }]}
          onPress={togglePump}  // Call the togglePump function
        >
          <Text style={styles.buttonText}>
            {isPumpOn ? "Turn Off Pump" : "Turn On Pump"}
          </Text>
      ...
      <View style={styles.section}>
        ...
        {isAutomationEnabled && (
          <View style={styles.scheduleContainer}>
            ...
            <TouchableOpacity
              style={[styles.scheduleItem, schedule.morning && styles.scheduleItemActive]}
              onPress={() =>
                setSchedule({ ...schedule, morning: !schedule.morning })
              }
            >
              ...
              />
              <Text
                style={[styles.scheduleText, schedule.morning && styles.scheduleTextActive]}
              >
                Morning (6:00 AM)
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[styles.scheduleItem, schedule.afternoon && styles.scheduleItemActive]}
              onPress={() =>
                setSchedule({ ...schedule, afternoon: !schedule.afternoon })
              }
            >
              ...
              />
              <Text
                style={[styles.scheduleText, schedule.afternoon && styles.scheduleTextActive]}
              >
                Afternoon (2:00 PM)
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[styles.scheduleItem, schedule.evening && styles.scheduleItemActive]}
              onPress={() =>
                setSchedule({ ...schedule, evening: !schedule.evening })
              }
            >
              ...
              />
              <Text
                style={[styles.scheduleText, schedule.evening && styles.scheduleTextActive]}
              >
                Evening (6:00 PM)
              </Text>
            </TouchableOpacity>
